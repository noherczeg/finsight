...
- When asked to commit, you MUST write the commit message in Conventional
  Commits style: `<type>[optional scope]: <description>`.
- Allowed `type`: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`,
  `build`, `ci`, `chore`, `revert`. Add `!` after type/scope for a breaking
  change (or a `BREAKING CHANGE:` footer).
- Subject: imperative mood, lowercase, no trailing period, ≤72 chars.
- Add a body when the change needs rationale; reference issues in a footer.
