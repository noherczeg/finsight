# portfolio-sandbox Specification

## Purpose
finsight's second browser simulation: an interactive multi-lens explorer that
makes investment risk and reward tangible. One shared asset model (allocation
weights + correlation + contribution plan) feeds seven lenses — asset
personalities, diversification, Sharpe, fees/inflation, a seeded Monte Carlo
cone, a historical time-machine, and behavior drills. Deterministic closed-form
core plus a reproducible (seeded) stochastic cone and deterministic replay of a
bundled real-returns dataset, with honest defaults and a not-financial-advice
disclaimer.
