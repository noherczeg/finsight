...
function App() {
    return (
        <main className="relative flex min-h-full flex-col overflow-hidden">
            ...
            />
            <div aria-hidden className="pointer-events-none absolute bottom-0 right-0 h-96 w-96 rounded-full bg-emerald-500/10 blur-[120px]" />

            <header className="relative z-10 mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6">
                <div className="flex items-center gap-2">
                    <span className="grid h-9 w-9 place-items-center rounded-lg bg-mint/15 font-mono text-lg text-mint-bright">f</span>
            ...
            <section className="relative z-10 mx-auto flex w-full max-w-4xl flex-1 flex-col items-center justify-center px-6 py-20 text-center">
                ...
                <h1 className="text-balance text-5xl font-bold leading-tight tracking-tight sm:text-7xl">
                    See the money <span className="bg-gradient-to-r from-mint-bright via-mint to-emerald-400 bg-clip-text text-transparent">move.</span>
                </h1>
                <p className="mt-6 max-w-2xl text-pretty text-lg text-slate-400">
                    finsight turns markets, compounding, and risk into hands-on browser simulations. No jargon, no spreadsheets to memorize — just realistic,
            ...
            <section id="scenarios" className="relative z-10 mx-auto grid w-full max-w-6xl gap-6 px-6 pb-24 sm:grid-cols-3">
                {scenarios.map((s) => (
                    <article
                        key={s.title}
            ...
            <footer id="about" className="relative z-10 border-t border-white/10 px-6 py-8 text-center text-sm text-slate-500">
                Built for the curious. finsight is an educational sandbox — not financial advice.
            </footer>
        </main>
    );
}
